...
var Dial = function(userCanvas,segments){
    ...
    function drawTextMarkers(options) {
        ...
        options.ctx.fillStyle = 'rgb(57,181,74)';
        ...
        for (iTick = 10; iTick < 180; iTick += 20) {
            ...
            if (iTick <= 10) {
                options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - tickOffset) + innerTickX,
                        (gaugeOptions.center.Y - gaugeOptions.radius - tickOffset) + innerTickY + 5);
            } else if (iTick < 50) {
                options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - tickOffset) + innerTickX - 5,
                        (gaugeOptions.center.Y - gaugeOptions.radius - tickOffset) + innerTickY + 5);
            } else if (iTick < 90) {
                options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - tickOffset) + innerTickX,
                        (gaugeOptions.center.Y - gaugeOptions.radius - tickOffset) + innerTickY);
            } else if (iTick === 90) {
                options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - tickOffset) + innerTickX + 4,
                        (gaugeOptions.center.Y - gaugeOptions.radius - tickOffset) + innerTickY);
            } else if (iTick < 145) {
                options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - tickOffset) + innerTickX + 10,
                        (gaugeOptions.center.Y - gaugeOptions.radius - tickOffset) + innerTickY);
            } else {
                options.ctx.fillText(iTickToPrint, (options.center.X - gaugeOptions.radius - tickOffset) + innerTickX + 15,
                        (gaugeOptions.center.Y - gaugeOptions.radius - tickOffset) + innerTickY + 5);
            }
    ...
    function drawSpeedometerPart(options, alphaValue, strokeStyle, startPos, endPos) {
        ...
        */
        ...
        options.ctx.arc(options.center.X,
            options.center.Y,
            options.levelRadius - 2,
            Math.PI + (Math.PI / 360 * startPos),
            !endPos ? (0 - (Math.PI / 360 * 10)) : (Math.PI + (Math.PI / 360 * endPos)),
            false);
...
//        drawSpeedometerPart(options, 0.9, "rgb(57, 181, 74)", startOfGreen);
    ...
    function drawNeedle(options) {
        ...
        */
        ...
        fromX = toX + 25*Math.sin(angle);
        fromY = toY + 25*Math.cos(angle);

        var line = createLine(fromX, fromY, toX, toY, "rgb(0,0,0)", 5, 0.6);
...
//            drawTextMarkers(options);
        ...
        triggerChange();
        job = setTimeout(draw, 20);
        
    }
    
    function set(value,instant){
        if (value !== null) {
            ...
            iTargetSpeed = value * 0.8;

            if(instant){
                iCurrentSpeed = iTargetSpeed;
            }
            job = setTimeout(draw, 0);
